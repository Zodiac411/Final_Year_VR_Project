...
public class VendingMachine : MonoBehaviour
{
    ...
    public void VendWeapon(int index)
    {
        ...
        if (slot != null)
        {
            ...
            print(price);
            if (CreditsManager.Instance.TrySpendCredits(price))
            {
    ...
    public void UpdateUIForSlot(WeaponSlot slot)
    {
        ...
        foreach (var slotUI in weaponSlotUIs)
        {
            ...
            {
                string label = slot.selectedOffer != null ? slot.selectedOffer.displayName : slot.selectedWeaponName;
                slotUI.textMeshProUGUI.text = $"{label}\n${slot.GetPrice()}";
                break;
            }
        }
    }
